...
- (id)initWithFrame:(CGRect)frame
     gameController:(UZChessGameController *)controller
          boardView:(UZChessBoardView *)bView {
   if (self = [super initWithFrame: frame]) {
      sqSize = bView.boardInnerView.squareSize;
...
- (void)hintAnimationFinished2:(NSString *)animationID
   ...
   [self setFrame: [boardView.boardInnerView rectForSquare: square]];
   [boardView.boardInnerView bringArrowToFront];
   [UIView commitAnimations];
}
...
- (void)hintAnimationFinished:(NSString *)animationID
   ...
   CGPoint pt2 = [boardView.boardInnerView originOfSquare: square];
...
- (void)moveToSquareAndBack:(Square)newSquare {
   ...
   [self setFrame: [boardView.boardInnerView rectForSquare: newSquare]];
   [UIView commitAnimations];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
   ...
   if ([boardView.boardInnerView fromSquare] != SQ_NONE)
      [boardView.boardInnerView touchesBegan: touches withEvent: event];
   else {
      [boardView.boardInnerView hideLastMove];
      if ([gameController usersTurnToMove] &&
         ...
         // the game controller?
         [boardView.boardInnerView highlightSquares: sqs];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
   if (isBeingDragged) {
      ...
      if (!wasDraggedAwayFromSquare &&
          [boardView.boardInnerView squareAtPoint: boardPt] != square) {
         wasDraggedAwayFromSquare = YES;
      }
      [boardView.boardInnerView selectionMovedToPoint:boardPt];
   }
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
   ...
   if (isBeingDragged) {
      ...
      Square releasedSquare = [boardView.boardInnerView squareAtPoint: releasePoint];

      if ([gameController pieceCanMoveFrom: square to: releasedSquare]) {
         [boardView.boardInnerView stopHighlighting];
         ...
         // method work.
         if (!wasDraggedAwayFromSquare) {
            [boardView.boardInnerView pieceTouchedAtSquare:square];
            return;
         }
         else
            [boardView.boardInnerView stopHighlighting];
